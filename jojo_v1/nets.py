...
def main():
    ...
    # create a client usin the SDK
    client_args = APIClientArgs(server=api_server)
    with APIClient(client_args) as client:
        # use the client to connect to the management server
        login = client.login(username, password)

        # If the login returns error, show the error and exit
        if not login.success:
            print(login.error_message)
            sys.exit(1)

        # pull existing nets from the defined group
        nets = client.api_call(
            "show-group", payload={"name": group_name, "details-level": "Full"}
        )
        ...
        for item in net_list:
            ...
            # changing naming convension, add zeros if needed
            octets = [f"{int(octet):03}" for octet in network_address.split(".")]
            ...
            if item["network"] in converted_nets:
                print(f"subnet {item['network']} exists on the management station")
                ...
                check_network_duplicate = client.api_call(
                    "show-network",
                    payload={"name": network_name, "details-level": "Full"},
                )
                ...
                if check_network_duplicate.success:
                    ...
                    net_mask_length = check_network_duplicate.data["mask-length4"]
                    net_comments = check_network_duplicate.data["comments"]

                    if (
                        net_name != network_name
                        or net_address != network_address
                        or net_mask_length != mask_length
                        or net_comments != description
                    ):
                    ...
                    check_network_duplicate.data["code"]
        ...
        add_network_to_group = client.api_call(
            "set-group", payload={"name": group_name, "members": members}
        )
        ...
        set_session = client.api_call(
            "set-session",
            payload={"new-name": session_name, "description": session_description},
        )
